...
class Builder extends Field implements Contracts\CanConcealComponents
{
     ...
     * @var view-string
     */
    protected string $view = "forms.components.builder-with-preview";

    protected string|Closure|null $addBetweenActionLabel = null;

    protected string|Closure|null $addActionLabel = null;

    // protected string|Closure|null $openPreviewActionLabel = null;
    ...
    // protected ?Closure $modifyOpenPreviewActionUsing = null;
    ...
    protected function setUp(): void
    {
        ...
        $this->registerActions([
            ...
            fn(
                Builder $component
            ): Action => $component->getCollapseAllAction(),
            ...
            // fn(
            //     Builder $component
            // ): Action => $component->getOpenPreviewAction(),
        ]);

        $this->mutateDehydratedStateUsing(static function (
            ?array $state
        ): array {
    ...
    // public function isPreviewOpen(): bool
    // {
    //     return false;
    // }

    // public function getOpenPreviewAction(): Action
    // {
    //     $action = Action::make($this->getOpenPreviewActionName())
    //         ->label(
    //             fn(
    //                 Builder $component
    //             ) => $component->getOpenPreviewActionLabel()
    //         )
    //         ->color("gray")
    ...
    //     if ($this->modifyOpenPreviewActionUsing) {
    //         $action =
    //             $this->evaluate($this->modifyOpenPreviewActionUsing, [
    //                 "action" => $action,
    //             ]) ?? $action;
    //     }
    ...
    // public function openPreviewAction(?Closure $callback): static
    // {
    //     $this->modifyOpenPreviewActionUsing = $callback;

    //     return $this;
    // }

    // public function getOpenPreviewActionName(): string
    // {
    //     return "openPreview";
    // }
    ...
    public function getAddAction(): Action
    {
        ...
        if ($this->modifyAddActionUsing) {
            $action =
                $this->evaluate($this->modifyAddActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getAddBetweenAction(): Action
    {
        $action = Action::make($this->getAddBetweenActionName())
            ->label(
                fn(Builder $component) => $component->getAddBetweenActionLabel()
            )
            ->color("gray")
            ->action(function (array $arguments, Builder $component): void {
                $newUuid = (string) Str::uuid();
    ...
    public function getCloneAction(): Action
    {
        ...
        if ($this->modifyCloneActionUsing) {
            $action =
                $this->evaluate($this->modifyCloneActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getDeleteAction(): Action
    {
        ...
        if ($this->modifyDeleteActionUsing) {
            $action =
                $this->evaluate($this->modifyDeleteActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getMoveDownAction(): Action
    {
        $action = Action::make($this->getMoveDownActionName())
            ->label(
                __("filament-forms::components.builder.actions.move_down.label")
            )
            ->icon("heroicon-m-arrow-down")
            ->color("gray")
            ->action(function (array $arguments, Builder $component): void {
                $items = array_move_after(
                    $component->getState(),
                    $arguments["item"]
                );
        ...
        if ($this->modifyMoveDownActionUsing) {
            $action =
                $this->evaluate($this->modifyMoveDownActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getMoveUpAction(): Action
    {
        $action = Action::make($this->getMoveUpActionName())
            ->label(
                __("filament-forms::components.builder.actions.move_up.label")
            )
            ->icon("heroicon-m-arrow-up")
            ->color("gray")
            ->action(function (array $arguments, Builder $component): void {
                $items = array_move_before(
                    $component->getState(),
                    $arguments["item"]
                );
        ...
        if ($this->modifyMoveUpActionUsing) {
            $action =
                $this->evaluate($this->modifyMoveUpActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getCollapseAction(): Action
    {
        ...
        if ($this->modifyCollapseActionUsing) {
            $action =
                $this->evaluate($this->modifyCollapseActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getExpandAction(): Action
    {
        ...
        if ($this->modifyExpandActionUsing) {
            $action =
                $this->evaluate($this->modifyExpandActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getCollapseAllAction(): Action
    {
        $action = Action::make($this->getCollapseAllActionName())
            ->label(
                __(
                    "filament-forms::components.builder.actions.collapse_all.label"
                )
            )
            ->color("gray")
            ->livewireClickHandlerEnabled(false)
        ...
        if ($this->modifyCollapseAllActionUsing) {
            $action =
                $this->evaluate($this->modifyCollapseAllActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function getExpandAllAction(): Action
    {
        $action = Action::make($this->getExpandAllActionName())
            ->label(
                __(
                    "filament-forms::components.builder.actions.expand_all.label"
                )
            )
            ->color("gray")
            ->livewireClickHandlerEnabled(false)
        ...
        if ($this->modifyExpandAllActionUsing) {
            $action =
                $this->evaluate($this->modifyExpandAllActionUsing, [
                    "action" => $action,
                ]) ?? $action;
        }
    ...
    public function openPreviewActionLabel(string|Closure|null $label): static
    {
        $this->openPreviewActionLabel = $label;
     ...
     */
    public function createItemBetweenButtonLabel(
    ...
    {
        $this->addable(
            fn(Builder $component): bool => !$this->evaluate($condition)
        );
    ...
    public function reorderableWithDragAndDrop(
    ...
    public function reorderableWithButtons(
        bool|Closure $condition = true
    ): static {
    ...
    public function getChildComponentContainers(bool $withHidden = false): array
    {
        return collect($this->getState())
            ->filter(
                fn(array $itemData): bool => $this->hasBlock($itemData["type"])
            )
            ->map(
                fn(
                    array $itemData,
                    $itemIndex
                ): ComponentContainer => $this->getBlock($itemData["type"])
    ...
    // public function getOpenPreviewActionLabel(): string
    // {
    //     return $this->evaluate($this->openPreviewActionLabel) ??
    //         __("builder.actions.openPreview.label", [
    ...
    {
        return $this->evaluate($this->addBetweenActionLabel) ??
            __("filament-forms::components.builder.actions.add_between.label");
    }

    public function getAddActionLabel(): string
    {
        return $this->evaluate($this->addActionLabel) ??
            __("filament-forms::components.builder.actions.add.label", [
                "label" => Str::lcfirst($this->getLabel()),
            ]);
    }
    ...
    {
        return $this->evaluate($this->isReorderableWithDragAndDrop) &&
            $this->isReorderable();
    }

    public function isReorderableWithButtons(): bool
    {
        return $this->evaluate($this->isReorderableWithButtons) &&
            $this->isReorderable();
    }
    ...
    public function isAddable(): bool
    {
        ...
        if (
            filled($this->getMaxItems()) &&
            $this->getMaxItems() <= $this->getItemsCount()
        ) {
            return false;
        }
    ...
    public function stylesheet($url): static
    {
        $this->stylesheet = $url;
        return $this;
    }
